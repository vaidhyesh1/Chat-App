  ...
  <div class="content-wrapper">
    ...
    <br>
             ...
             <div class="wrap-input100 validate-input">
						<input type="text" class="input100" type="text" name="username" placeholder="Enter Username to send">
            ...
            <div class="wrap-input100 validate-input">
						<input type="number" min="1" step="any" class="input100" type="text" name="rate" placeholder="Enter Amount">
						<span class="focus-input100"></span>
						<span class="symbol-input100">
							<i class="fa fa-bank" aria-hidden="true"></i>
						</span>
...
if(isset($_POST['submit'])){
	...
	$stmt = $mysqli1->prepare("Select money from logincred  WHERE username = '".$_SESSION['user']."'") ; 
  $stmt->execute();	
  $stmt->bind_result($money); 
  $stmt->fetch(); 
  $stmt->close();  
  $money=(int)$money;
  if($money>=(int)$_POST['rate'])
  {
    $money=$money-(int)$_POST['rate'];
    $stmt3 = $mysqli1->prepare("select money from logincred  WHERE username = '".$_POST['username']."';");
    $stmt3->execute();	 
    $stmt3->bind_result($result);     
    $stmt3->fetch();     
    $stmt3->close();
    if($result!=null){
    $stmt2 = $mysqli1->prepare("Update logincred set money='".((int)$result+(int)$_POST['rate'])."'  WHERE username = '".$_POST['username']."';"); 
    $stmt2->execute();	
    $stmt2->close();
    ...
    echo '<script>alert("Money Sent!");</script>';    
      ...
      echo '<html><script>alert("Transaction Failed!");</script></html>';    
    }
